...
def extract_audio(dir, video_file):
    ...
    video_input = os.path.join(dir, video_file)
#     cmd = ["avconv", "-y", "-i", video_input, "-vn", "-ac", "1", "-f", "wav", output]
    cmd = "ffmpeg -y -i {} -vn -ac 1 -f wav {}".format(video_input, output)
...
def align(video1, video2, dir, fft_bin_size=1024, overlap=0, box_height=512, box_width=43, samples_per_box=7):
    ...
    print("Finished processing second file")
    
    # Determie time delay
    pairs = find_freq_pairs(ft_dict1, ft_dict2)
    print("Freq pairs found")
